...
void main(void)
{
	...
	initPins();							// Init Pin Functionality
  	...
  	calibrate();	//Call function to calibrate
  					//corners of the screen

  	fillScreen(WHITE);

  	//Draw button
  	fillRectangle(BBEGIN, BEND, YBEGIN, YEND, BLU);
  	startX = BBEGIN + 7;
  	startY = YBEGIN + 20;
	...
	while (1)
	{
		...
		pc = calibratePoint(p); // change points to their screen
								// equivalents

		if (pc.z > 20)		//If screen is pressed
		{
			if (pc.y > YBEGIN && pc.y < YEND)
			{
				// If screen press on button
				if (pc.x > BBEGIN && pc.x < BEND)
				{
					fillRectangle(0, 239, 0, YBEGIN, WHITE);
					blinkLED();
				}
				else //Draw Circle
					fillCircle(pc.x, pc.y, 3, RED);
			}
			else // Draw Circle
				fillCircle(pc.x, pc.y, 3, RED);
		}
...
void blinkLED(void)
{
	isChanged = 0;	//No changes have occured yet.
	left = 1;		//Turn on left light
	center = 1;		//Turn on center light
	right = 1;		//Turn on right light
	gcd = 0;		//Set baseline counter to zero

	P1DIR |= 0x40;		// P1.0 Output
	P1OUT |= 0x40;		// Turn on light
	initClkSlow();	 	//Slow down the clock to 1MHz
	TACCTL0 = 0x0010;   // CCIE interrupt enabled
	TACCR0  = 0x2800;   // CCR = 0x2800
	TACTL   = 0x02D0;	// SMCLK, /8 Down Mode
	_BIS_SR(GIE);

	while(1)
		{
			if (isChanged) //If clock triggered change
			{
				drawLED(left, center, right);
				isChanged |= 0;
			}
		}
}
...
{
	//Increment greatest common denominator
	gcd++;

	// Blink LED every 1/6 second
	P1OUT ^= 0x40;
	//Every 2/3 second
	if (gcd % 4 == 0)
	{
...
void drawLED(int left, int center, int right)
{
	//Draw LED lights, if variables aren't 0
	// If zero, then draw dark LED
	if (left)
		fillCircle(60, 60, 10, YELLOW);
	else
		fillCircle(60, 60, 10, DARKYELLOW);

	if (center)
		fillCircle(120, 60, 10, YELLOW);
	else
		fillCircle(120, 60, 10, DARKYELLOW);

	if (right)
		fillCircle(180, 60, 10, YELLOW);
	else
		fillCircle(180, 60, 10, DARKYELLOW);
}
...
void initClkSlow(void)
{
   BCSCTL1 = CALBC1_1MHZ;				// 16 MHz Operation
   DCOCTL = CALDCO_1MHZ;
}
