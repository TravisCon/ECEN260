...
#define JOY_X		0x0004		// X axis Joystick
#define JOY_Y		0x0006		// Y axis Joystick
...
void main(void)
{
	...
	{
		waitMS(100);
		joy = getJSPos(); //Get the x and y values

		// First half of the formula
		// voltage.x = (x * 3.3) / 1024
		float x1 = (joy.x * 3.3);
		float y1 = (joy.y * 3.3);

		Vx = x1 / 1024; //2nd half of formula
		Vy = y1 / 980;

		startX = 75;
		startY = 148;
		writeWords("X volts ", BLK, 1, WHITE);
		fToC(cNumber, Vx);
		writeWords(cNumber, BLK, 1, WHITE);

		startX = 75;
		startY += 8;
		writeWords("Y volts ", BLK, 1, WHITE);
		fToC(cNumber, Vy);
		writeWords(cNumber, BLK, 1, WHITE);
	}
}
